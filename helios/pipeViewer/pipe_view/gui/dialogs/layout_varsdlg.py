...
class VarsListCtrl(wx.ListCtrl,
    ...
    # (or current values every time the user changes the Element being edited)
    ...
    def __UpdateItem(self, index: int) -> bool:
        ...
        self.SetItemBackgroundColour(index,
                                     self.GetItemBackgroundColour(index))

        if not self.__vars:
            # Set to no value. Whole window should be disabled
            return super().SetItem(index, 1, '')
...
class LayoutVariablesDialog(wx.Frame):
    ...
    def __init__(self,
                 ...
                 id: int,
                 title: str,
                 layout_context: Layout_Context) -> None:
