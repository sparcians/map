...
class Element_PropsDlg(wx.Frame):
    ...
    def __init__(self, parent: Layout_Frame, id: int, title: str) -> None:
        ...
        self.__fnt_location = wx.Font(ScaleFont(12),
                                      wx.NORMAL,
                                      wx.NORMAL,
                                      wx.NORMAL)
    ...
    def Draw(self, evt: Optional[wx.TimerEvent] = None) -> None:
        ...
        self.SetStatusText(f'{self.__list.GetNumberOfElements()} elements')
    ...
    # multiple selections
    # @note This must be the only method through which elements on which the
    # dialog should operate can be set
    ...
    def __OnKeyDown(self, evt: wx.KeyEvent) -> None:
        ...
        if evt.GetKeyCode() == ord('Z') and ctrl:
            self.__parent.GetCanvas().GetSelectionManager().Undo()
        elif evt.GetKeyCode() == ord('Y') and ctrl and evt.ShiftDown():
            self.__parent.GetCanvas().GetSelectionManager().RedoAll()
        elif evt.GetKeyCode() == ord('Y') and ctrl:
...
# Kind of fits in this module. It's small code so I'd rather not give it its
# own module.
class ElementTypeSelectionDialog(wx.Dialog):
    ...
    def __init__(self, parent: Layout_Canvas) -> None:
        ...
        self.__drop_down = wx.ComboBox(self,
                                       wx.NewId(),
                                       choices=self.creatables,
                                       style=wx.TE_PROCESS_ENTER)
        # assume always one creatable
        self.__drop_down.SetStringSelection(self.creatables[0])
    ...
    def OnDone(self, evt: wx.CommandEvent) -> None:
        idx = int(self.__drop_down.GetSelection())
        self.__type_string = self.creatables[idx]
        # close off selection
