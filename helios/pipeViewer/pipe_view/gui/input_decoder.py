...
class Input_Decoder:
    ...
    def LeftDown(self,
        ...
        event_pos = canvas.ScreenToClient(
            cast(wx.Window,
                 event.GetEventObject()).ClientToScreen(event.GetPosition())
        )

        pos = canvas.CalcUnscrolledPosition(event_pos)

        # small routine for playback mode
        if not self.__edit_mode:
            hits = canvas.context.DetectCollision(pos,
        ...
        # LeftDown logic. Fun!
        resize_evt = selection.HitHandle(pos)
        if resize_evt:
        ...
        # NOTE: pressing shift will short-circuit all other LeftDown logic.
        if event.ShiftDown() and not event.ControlDown() and len(hits) > 0:
            ...
            top_hit_pos = cast(Tuple[int, int],
                               top_hit.GetProperty('position'))
            top_hit_dim = cast(Tuple[int, int],
                               top_hit.GetProperty('dimensions'))
            exes = [top_hit_pos[0]]
            whys = [top_hit_pos[1]]
            ...
            tl_pos = (min(exes), min(whys))
            br_pos = (max(exes), max(whys))
            selection.SetPos(tl_pos)
            add_these = selection.CalcAddable(br_pos)
            for el in add_these:
                selection.Add(el)
            ...
            corner_case = 0
            if corner_case == 0:
                # Trust me, it is indeed supposed to be Addable(), not
                # Removable()
                remove_these = selection.CalcAddable(pos)
        ...
        # Single Element under mouse click
            ...
            # Selection Manager is responsible for updating the properties
            # dialog
        ...
        # No Elements under mouse click
            ...
            if not (event.ControlDown() and event.ShiftDown()):
                selection.ProcessRubber(pos, selection.add)
            else:
                selection.ProcessRubber(pos, selection.remove)
    ...
    def LeftUp(self,
        ...
        if hist == selection.prep_remove:
            pos = canvas.CalcUnscrolledPosition(event.GetPosition())
            hits = canvas.DetectCollision(pos)
            ...
            if len(hits) > 0:
                selection.SetHistory(selection.collision)
                if len(hits) == 1 or \
                   selection.IsSelected(hits[0].GetElement()):
                    selection.Clear()
                ...
                print("this is some weird case of the clicks that isn't supposed to be possible...")  # noqa: E501
                print("you should probably make note of what you just did, and file a bug")  # noqa: E501
    ...
    def KeyDown(self,
        ...
        if not self.__edit_mode:
            # Playback Mode
            playback_panel = self.__parent.GetFrame().GetPlaybackPanel()
            if key in key_definitions.STEP_FORWARD:
                if not self.__is_traveling:
                    self.__is_traveling = True
                    if event.ShiftDown():
                        # move multiple spaces
                        playback_panel.StepForward(self.__SHIFT_STEP_DISTANCE)
                    else:
                        playback_panel.StepForward()  # move one space
                    playback_panel.StartPlaying(9, delay=1)
                key_handled = True
                ...
                if not self.__is_traveling:
                    ...
                    if event.ShiftDown():
                        # move multiple spaces
                        playback_panel.StepBackward(self.__SHIFT_STEP_DISTANCE)
                    else:
                        playback_panel.StepBackward()  # move one space
                    playback_panel.StartPlaying(-9, delay=1)
                key_handled = True
                ...
                if not self.__is_traveling:
                    ...
                    if event.ControlDown():
                        # move (more) multiple spaces
                        playback_panel.StepForward(self.__CTRL_STEP_DISTANCE)
                    else:
                        # move multiple spaces
                        playback_panel.StepForward(self.__SHIFT_STEP_DISTANCE)
                    playback_panel.StartPlaying(9, delay=1)
                ...
                if not self.__is_traveling:
                    ...
                    if event.ControlDown():
                        # move (more) multiple spaces
                        playback_panel.StepBackward(self.__CTRL_STEP_DISTANCE)
                    else:
                        # move multiple spaces
                        playback_panel.StepBackward(self.__SHIFT_STEP_DISTANCE)
                    playback_panel.StartPlaying(-9, delay=1)
