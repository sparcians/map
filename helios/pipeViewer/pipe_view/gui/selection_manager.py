...
class Selection_Mgr():
    ...
    # # Calculates the actual resize operation, relative to the data in the
    #  queue,  and saves results to the queue
    def __RightResize(self, pt, base):
        bx, _, bw, bh = self.__queue[base]
        delta = pt[0] - self.__prev_x
    ...
    def __BottomResize(self, pt, base):
        '''
        Calculates the actual resize operation, relative to the data in the
        queue,  and saves results to the queue
        '''
    ...
    def __TopResizeScaling(self, pt, base):
        '''
        ...
        if (((-delta < 0 and by > pt[1]) or (-delta > 0 and by < pt[1])) and
            ...
            if mh >= self.__c_box_wid * 2:
                for e in self.__selected:
                    x, y, w, h = self.__queue[e]
                    y = (self.__bottom - y) * factor + self.__bottom
                    h = -h * factor
    ...
    def __LeftResizeScaling(self, pt, base):
        ...
        if (((-delta < 0 and bx > pt[0]) or (-delta > 0 and bx < pt[0])) and
            ...
            if mw >= self.__c_box_wid * 2:
                for e in self.__selected:
                    x, y, w, h = self.__queue[e]
                    x = (self.__right - x) * factor + self.__right
                    w = -w * factor
    ...
    def __RightResizeScaling(self, pt, base):
        ...
        if (((delta < 0 and (bx + bw) > pt[0]) or (delta > 0 and (bx + bw) < pt[0]))
            ...
            if mw >= self.__c_box_wid * 2:
                for e in self.__selected:
                    x, y, w, h = self.__queue[e]
                    x = (x - self.__left) * factor + self.__left
                    w = w * factor
                    w = max(w, self.__c_box_wid * 2)
                    self.__queue[e] = (x, y, w, h)
        self.__prev_x = pt[0]
    ...
    def __TopMidResizePrep(self, base):
        bh = base.GetYDim()
        by = base.GetYPos()
        self.__top = by
        self.__bottom = by + bh
    ...
    #  of the selection, find the smallest Element in the corresponding
    #  dimension. Called on MouseDown before the operation actually takes place
    def __LeftMidResizePrep(self, base):
        bw = base.GetXDim()
        bx = base.GetXPos()
        self.__min_wid = base
        self.__left = bx
    ...
    #  of the selection, find the smallest Element in the corresponding
    #  dimension. Called on MouseDown before the operation actually takes place
    def __RightMidResizePrep(self, base):
        bw = base.GetXDim()
        bx = base.GetXPos()
        self.__min_wid = base
        self.__left = bx
    ...
    #  of the selection, find the smallest Element in the corresponding
    #  dimension. Called on MouseDown before the operation actually takes place
    def __BottomMidResizePrep(self, base):
        bh = base.GetYDim()
        by = base.GetYPos()
        self.__top = by
        self.__bottom = by + bh
    ...
    # # Forwarder method
    def __TopLeftResizePrep(self, base):
        self.__TopMidResizePrep(base)
        self.__LeftMidResizePrep(base)

    # # Forwarder method
    def __TopRightResizePrep(self, base):
        self.__TopMidResizePrep(base)
        self.__RightMidResizePrep(base)

    # # Forwarder method
    def __BottomRightResizePrep(self, base):
        self.__BottomMidResizePrep(base)
        self.__RightMidResizePrep(base)

    # # Forwarder method
    def __BottomLeftResizePrep(self, base):
        self.__LeftMidResizePrep(base)
        self.__BottomMidResizePrep(base)
    ...
    def __init__(self, canvas, elpropsdlg):
        ...
        self.__selected = {}
        self.__playback_selected = None
        self.__is_edit_mode = False
        self.__dominant = None
        self.__position = (0, 0)
        self.__history = -1
        self.__resize_direction = ()
        ...
        self.__cur_open_chkpt = None # Current move action checkpoint. To be closed after the move

        self.__undo_redo_hooks = [] # Hooks for notifying clients about undo/redo.
    ...
    def __Checkpoint(self, description, force_elements = None):
    ...
    def SetEditMode(self, is_edit_mode):
    ...
    def GenerateElement(self, layout, type_string, add_to_selection = False):
    ...
    def GenerateDuplicateSelection(self, layout, delta):
        if not self.__copy_completed:
            ...
            for e in original_selection:
                ce = layout.CreateAndAddElement(e, element_type = e.GetProperty('type'))
                x, y = ce.GetProperty('position')
    ...
    def InvertSelection(self, layout):
    ...
    def BeginCheckpoint(self, description, **kwargs):
    ...
    #  @param force_elements (kwarg only) Begins using a specific set of
    #  elements. These elements will be treated as if they were selected
    def CommitCheckpoint(self, **kwargs):
    ...
    def Move(self, pos = None, delta = None, **kwargs):
        force_no_snap = False
    ...
    def Indent(self, base, subtractive = False):
    ...
    #  appended to its end
    def __FindPINsUntilMatches(self, elements, set_to_match, incomplete_set_append, full_set = True):
    ...
    def GetBoundingBox(self):
        ...
        for e in self.__selected:
            ...
            if x < l or l is None:
                l = x
            if x + w > r or r is None:
                r = x + w
            if y < t or t is None:
                t = y
            if y + h > b or b is None:
                b = y + h
    ...
    def ProcessRubber(self, xxx_todo_changeme, action = None):
        ...
        if self.__which_rubber_band == self.add:
            add_these = self.CalcAddable((x, y))
            self.Add(add_these)
            ...
            self.__temp_rubber = remove_these
