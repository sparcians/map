...
class QuadNode:
    ...
    def GetAllChildElements(self) -> List[Element_Value]:
        # contains duplicates
        l: List[Element_Value] = []
        self.__GetAllChildren(self, l)
        return l

    def __GetAllChildren(self,
                         node: QuadNode,
                         output_list: List[Element_Value]) -> None:
...
class QuadTree:
    ...
    def __init__(self) -> None:
        ...
        # e.g. self.__lookup_table[myObj] = [QuadNode_instance1, QuadNode_instance2]  # noqa: E501
    ...
    def AddSingleToTree(self,
                        obj: Element_Value,
                        no_remove: bool = False) -> None:
        lookup_entry = self.__lookup_table.get(obj)
        if lookup_entry:
    ...
    def Build(self,
              update: Optional[List[Element_Value]] = None,
              no_clear: bool = False) -> QuadNode:
        if update is None:
            update = []
        ...
        if update:
            ...
            self.__tree = QuadNode((min_x, min_y, max_x, max_y),
                                   contents=[],
                                   elements=self.__objects[:])
        current_nodes = [self.__tree]
        while current_nodes:
            # filter the objects down through tree
            node = current_nodes.pop(0)
            if node.elements and \
               (node.bounds[2] - node.bounds[0]) / 2.0 >= self.WIDTH_LIMIT:
                # will only create if needed
                node.BisectCreateNodes()
                while node.elements:
                    obj = node.elements.pop()
                    # accessing variable rather than dict (e.g. obj.pos) saves
                    # very little time
                    bounds = obj.GetElement().GetBounds()
    ...
    def GetObjects(
        self,
        given_bounds: Tuple[int, int, int, int]
    ) -> Set[Element_Value]:
    ...
    def __GetObjects(
        ...
        node: QuadNode,
        given_bounds: Tuple[int, int, int, int],
        output_set: Set[Element_Value]
    ) -> None:  # left, upper, right, lower
        if not node.contents:  # leaf
            ...
            for i in (0, 1, 2, 3):
                ...
                # in bounds
                x_intersects = ((given_bounds[0] <= cnode.bounds[2]) and
                                (given_bounds[2] >= cnode.bounds[0]))
                y_intersects = ((given_bounds[1] <= cnode.bounds[3]) and
                                (given_bounds[3] >= cnode.bounds[1]))
            ...
            for i in (0, 1, 2, 3):
                ...
                if (x_intersects and y_intersects):
                    if (given_bounds[0] <= cnode.bounds[0] and
                            ...
                            given_bounds[2] > cnode.bounds[2] and
                            given_bounds[3] > cnode.bounds[3]):
                        # Update all objets because the node is completely
                        # contained within the given bounds
                        cnode.SetVisibilityTickOnAllChildElements(vis_tick)
                    else:
