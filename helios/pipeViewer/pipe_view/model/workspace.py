...
class Workspace:
    ...
    def __init__(self, settings: ArgosSettings) -> None:
        ...
        # Weak references to frames
        self.__frames: List[weakref.ReferenceType[Layout_Frame]] = []
        ...
        self.__builtin_resource_dir = os.path.join(
    ...
    def OpenLayoutFrame(self,
        ...
        if lf is None:
            lf = ''
        # If '', no layout will be loaded (as the user intended)
        layout = Layout(lf, workspace=self)

        # Jump to startup tick on construction
        lc = Layout_Context(layout, db, start_tick, loc_vars)

        frame = Layout_Frame(self,
                             lc,
                             update_enabled,
                             title_prefix,
                             title_override,
        ...
        if not norefresh:
            frame.Show(True)
            # Updates meta-data so that everything is good on the first
            # rendering
            lc.RefreshFrame()
    ...
    def AddUserResourceDir(self, dirname: str) -> None:
        self.__user_resource_dirs.append(
    ...
    # @param filename Filename to find in resource dirs. If absolute path,
    # does not check resource dirs
    # @param ignore Single string or a sequence of stringsd. Should some
    # resolved resource paths be skipped? If this was invoked and the file
    # path it returns was not a valid file or could not be opened, then this
    # method can be re-invoked with that path (or a sequence of paths) in
    # order to ignore them. This is done by comparing canonical paths
    ...
    # points to exists (it maybe not be the correct resource though).
    # Otherwise returns None to indicate a failed resolution.
    def LocateResource(
        self,
        filename: str,
        ignore: Union[str, List[str]] = [],
        try_first: Optional[Union[str, Tuple[str, ...], List[str]]] = None
    ) -> Optional[str]:
        ...
        for idx, f in enumerate(self.__frames):
            ...
            if frame is not None:
                try:
                    ...
                    print(f'Failed to access workspace frame idx {idx}: '
                          f'{frame}. It was already destroyed',
                          file=sys.stderr)
        ...
        while len(self.__frames) > 0:
            ...
            if frame:
                try:
                    ...
                    print(
                        f'Failed to access {frame}. It was already destroyed',
                        file=sys.stderr
                    )

            # Should have been removed by _HandleClose. If it wasn't (due to
            # exception or other bug) remove it anyway
            if framewr in self.__frames:
                self.__frames.remove(framewr)

    # Returns a 2-tuple window position based on the existing windows
    def GetNextNewFramePosition(
        self,
        ...
        if self.__last_window_pos is not None:
            ...
            pos = (int(self.__primary_window_coords[0] +
                       (self.__primary_window_size[0]/2 - size[0]/2)),
                   int(self.__primary_window_coords[1] +
                       (self.__primary_window_size[1]/2 - size[1]/2)))
            # \todo Ensure that this is actually ON the screen. Should also
            # adjust if the window is largely off the screen
            self.__last_window_pos = pos
            return pos
        else:
    ...
    def __RemoveFrame(self,
                      frame: weakref.ReferenceType[Layout_Frame]) -> None:
    ...
    # Updates settings and propagates them to the rest of the model if
    # necessary
    def UpdateSettings(self, new_settings: Dict[str, Any]) -> None:
        ...
        if 'playback_font_size' in new_settings:
            update_funcs.append(
                lambda f: f().GetPlaybackPanel().UpdateFontSize()
            )
