...
class DatabaseHandle:
    ...
    def __init__(self, db: Database) -> None:
        ...
        if not isinstance(db, Database):
            raise TypeError(
                f'db must be an instance of database.Database, is a {type(db)}'
            )
    ...
    def query(self,
              ...
              end_inc: int,
              cb: Callable,
              mod_tracking: bool = True) -> None:
        assert end_inc >= start_inc, \
            'Query range must be ordered such that start <= end'

        # Handles queries with negative starts by invoking the callback for
        # each one because the update algorithm requires a callback at each
        # tick
        if start_inc < 0:
            # Allow fake callbacks to be made here which contain no
            # transactions
    ...
    def __str__(self) -> str:
        return f'<DatabaseHandle database="{self.database}">'
