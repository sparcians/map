...
class Checkpoint:
    ...
    #@profile
    def __init__(self, layout: Layout, before: List[Element], desc: str) -> None:
        assert layout is not None
        self.__layout = layout
        self.__description = desc
        self.__before_pins = [el.GetPIN() for el in before]
        self.__before = [self.__layout.CreateElement(el, initial_properties = el.GetProperties(), element_type=cast(str, el.GetProperty('type')), parent=el.GetParent()) for el in before]
    ...
    def after(self, after: List[Element]) -> None:
        ...
        self.__after = [self.__layout.CreateElement(el, initial_properties = el.GetProperties(), element_type=cast(str, el.GetProperty('type')), parent=el.GetParent()) for el in after]
    ...
    def apply(self) -> Tuple[int, List[Element]]:
        ...
        for el,pin,follows_pins in zip(self.__after, self.__after_pins, self.__after_follows_pins):
            if self.__layout.FindByPIN(pin) is not None:
                ...
                parent = el.GetParent()
                new_el = self.__layout.CreateAndAddElement(el, initial_properties = el.GetProperties(), element_type=eltype, parent=parent, force_pin=pin, follows_pins=follows_pins, skip_list = parent is not None)
