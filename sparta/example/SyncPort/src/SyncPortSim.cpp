// <SyncPortSimulatorn.cpp> -*- C++ -*-


#include <iostream>

#include "SyncPortSim.hpp"
...
SyncPortSimulator::SyncPortSimulator(sparta::Scheduler & scheduler, bool be_noisy) :
    sparta::app::Simulation("sparta_syncport_example", &scheduler),
    be_noisy_(be_noisy),
    clock_manager_(&scheduler)
{
...
SyncPortSimulator::~SyncPortSimulator()
{
    if(SPARTA_EXPECT_FALSE(be_noisy_)) {
...
void SyncPortSimulator::buildTree_()
{
    if(SPARTA_EXPECT_FALSE(be_noisy_)) {
    ...
    // Create two clocks
    const double producer_freq_mhz = 300;
    const double consumer_freq_mhz = 200;
    root_clk_     = clock_manager_.makeRoot(getRoot(), "root_clk");
    producer_clk_ = clock_manager_.makeClock("producer_clk", root_clk_, producer_freq_mhz);
    consumer_clk_ = clock_manager_.makeClock("consumer_clk", root_clk_, consumer_freq_mhz);
...
void SyncPortSimulator::configureTree_()
{
    if(SPARTA_EXPECT_FALSE(be_noisy_)) {
...
void SyncPortSimulator::bindTree_()
{
    if(SPARTA_EXPECT_FALSE(be_noisy_)) {
        std::cout << "NOISE: " << __PRETTY_FUNCTION__
