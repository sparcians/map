// <NestedPeventCollector> -*- C++ -*-


/**
 * \file NestedPeventCollector
 * \brief Contains A SpartaKeyPair collection implementation of a pevent
 * collector.
 */

#ifndef __NESTED_PEVENT_KEY_PAIR_COLLECTOR__
#define __NESTED_PEVENT_KEY_PAIR_COLLECTOR__
...
namespace pevents {
     ...
     * \tparam the Type of Actual Collectable class that we want to capture a pevent from.
     */
    ...
    class NestedPeventCollector :
        ...
        using PairCollector<PairDef_t>::turnOff_;
        using PairCollector<PairDef_t>::collect_;

    public:
        using PairCollector<PairDef_t>::isCollecting;
        NestedPeventCollector(const std::string& event_name,
            sparta::TreeNode* parent,
                const Clock* clk,
                    const bool verbosity=false) :
            sparta::PairCollector<PairDef_t>(),
            PeventCollectorTreeNode(parent,
                event_name+PEVENT_COLLECTOR_NOTE+(verbosity ? "_verbose" : "")),
            ...
            if(( lowertype == lower_ev_name || lowertype == "all") && verbosity_ == verbose) {
                ...
                for(const auto& existing_tap : taps_) {
                    if(existing_tap->getDestination()->compareStrings(file) &&
                        (existing_tap->getCategoryName() == lowertype
                            || lowertype == "all")) {
                        is_unique = false;
                        break;
                    }
...
#endif // __NESTED_PEVENT_KEY_PAIR_COLLECTOR__
