...
This is highly discouraged -- as resources should communicate through
Ports, not directly.  Instead, resources should communicate through
https://sparcians.github.io/map/communication.html#comm_ports[`sparta::Ports`].
...
stage allowing the modeler define the hierarchy, configure the
...
https://sparcians.github.io/map/classsparta_1_1app_1_1Simulation.html#a319e9c4e7418f8f953e1a527b4feff8a[sparta::app::Simulation::buildTree()]
in the public API:
...
been instantiated at end of this phase.
...
namespace my_simulator
{
    ...
    void MySimulator::buildTree_()
    {
        ...
        // the sake of brevity, assume a simulation of just 2 cores.  This
...
`sparta::ResourceTreeNode` found in the tree node hierarchy.
...
namespace my_simulator
{
    ...
    void MySimulator::configureTree_()
    {
        ...
        const sparta::Parameter<uint32_t> & num_branch_units =
         ...
         sparta_assert(num_branch_units == 1, "This core design only supports 1 branch unit");
    }
}
----
...
effort, but the phase is still started by the user by calling
...
created, these components are essentially ready for execution.
However, communication _between_ components (especially those with
`sparta::Ports`) is not yet established.  These connections are
performed in the next/final phase called the Bind Phase of simulation.
...
previous phase are now ready to be bound together.  This binding is
only pertinent for those components that have `sparta::Port` objects
or `sparta::Bus` objects that need to be connected.  This is the heart
of simulation for a Sparta-based model where data is
communicated/passed from one component to another.

Binding is the responsibility of the modeler and not the framework.
The framework only holds the components necessary for binding to ease
the process of binding, but is unaware of how components should be
connected.

Binding happens in the modeler's derivation of `bindTree_` virtual
method of `sparta::app::Simulation`.  Ports are found using the Ports name

To bind two `sparta::Port` or `sparta::Bus` objects together, the
following rules apply:

1. One of the Ports *must be* an `sparta::InPort` type and the other *must be*
a `sparta::OutPort` type
1. If the Port is a data type
(`sparta::DataInPort`/`sparta::DataOutPort`) the data type *must be*
equivalent and not castable
1. If the components are `sparta::Bus` types, they must be equivalent
in size and polarity.  For example, if the Bus has 1 InPort and 2
OutPorts, the Bus being bound to must have 1 OutPort and 2 InPorts of
the same name.
...
The last part of simulation configuration is to tell the Sparta
framework it can now "lock" the simulation environment and expect no
more topological changes.  This is done through the call to
https://sparcians.github.io/map/classsparta_1_1app_1_1Simulation.html#a18ede64ec543df1bb56a71618fe82757[`sparta::app::Simulation::finalizeFramework()`]
...
int main()
{
    ...
    my_sim.configureTree(); // Configure/adjust parameters before instantiation
    my_sim.finalizeTree();  // Finialize and lock in parameters; instantiate the model
    my_sim.finalizeFramework(); // Tell Sparta that simulation is ready to run
    return 0;
}
----
